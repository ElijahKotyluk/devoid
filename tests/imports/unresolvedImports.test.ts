...
const fixturesRoot = path.join(__dirname, "fixtures", "unresolved");
...
test("unresolved imports: external modules do not affect export usage", () => {
  ...
  assert.deepStrictEqual([...used[localFile]].sort(), ["localFn"]);
  assert.deepStrictEqual([...unused[localFile]], []);
});
